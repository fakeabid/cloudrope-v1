...
import { Star, Download, Share2, Trash2, ArrowRight } from 'lucide-react';
...
export default function Favorites() {
  ...
  const { items: allFiles, status } = useSelector((s) => s.files);
  ...
  // Only show files that are both active and pinned
  const pinnedFiles = allFiles.filter((f) => favoriteIds.includes(f.id));

  const { sortKey, setSortKey, filterKey, setFilterKey, processed } =
    useSortFilter(pinnedFiles, FILE_SORT_OPTIONS, FILE_FILTER_OPTIONS, 'date_desc');
  ...
  return (
    <div className="p-6 md:p-8 max-w-5xl">
      <div className="mb-5">
        ...
        <p className="text-text-muted text-sm mt-0.5">{pinnedFiles.length} pinned file{pinnedFiles.length !== 1 ? 's' : ''}</p>
      </div>

      {pinnedFiles.length > 0 && (
        <div className="flex flex-wrap items-center gap-3 mb-4">
          <SearchBar query={query} setQuery={setQuery} placeholder="Search favorites…" className="max-w-xs" />
          <SortFilterBar
            sortOptions={FILE_SORT_OPTIONS}
            sortKey={sortKey} setSortKey={setSortKey}
            filterOptions={FILE_FILTER_OPTIONS}
            filterKey={filterKey} setFilterKey={setFilterKey}
          />
        </div>
      )}

      <div className="bg-surface border border-border rounded-xl overflow-hidden">
        <div className="hidden md:grid grid-cols-[1fr_100px_120px_140px] items-center px-4 py-2.5 border-b border-border bg-elevated/40">
        ...
        {status === 'succeeded' && pinnedFiles.length === 0 && (
          <div className="flex flex-col items-center gap-3 py-20 text-center">
            ...
            <div>
              <p className="text-text-primary text-sm font-medium">No favorites yet</p>
              <p className="text-text-muted text-xs mt-0.5">Pin files from My Files to find them here quickly.</p>
            </div>
            <Link to="/dashboard/files" className="btn-secondary mt-1 text-xs">
              <ArrowRight size={13} />
              Go to My Files
            </Link>
          </div>
        ...
        {status === 'succeeded' && pinnedFiles.length > 0 && filtered.length === 0 && (
          <div className="flex flex-col items-center gap-2 py-12 text-center">
            <p className="text-text-muted text-sm">No favorites match "<span className="text-text-primary">{query}</span>"</p>
        ...
        {status === 'succeeded' && paged.map((file) => (
          <div
            key={file.id}
            className="grid grid-cols-[1fr_auto] md:grid-cols-[1fr_100px_120px_140px] items-center gap-3 px-4 py-3.5 border-b border-border last:border-0 hover:bg-elevated/30 transition-colors"
          >
